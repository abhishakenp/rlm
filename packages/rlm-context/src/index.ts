...
const CONTEXT_DOCTRINE = [
	...
	"",
	"Everything that goes into your context is a variable, and you name the ones you create.",
	"",
	"The harness has already created the ones only it can know, as immutable:",
	"`runtime.model`, `runtime.tools`, `runtime.depth`, `runtime.skills`,",
	"`runtime.systemPrompt`, `session.*`, `skill.*`, and `user.prompt`, which is captured",
	"the moment a prompt arrives. Setting any of them is refused — they exist and are",
	"immutable — and rewording the call does not change that. Read them instead:",
	"`const model = context.get('runtime.model')`.",
	"",
	"1. After EVERY tool execution, IMMEDIATELY and AUTOMATICALLY store the result with a MEANINGFUL name you choose (no reminder needed):",
	...
	"",
	"```js",
	"// Runtime state is already there — read it, never set it.",
	"const model = context.get('runtime.model');",
	"",
	"// User asks: 'list packages' — user.prompt was captured for you.",
