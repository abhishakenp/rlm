...
export class RlmPluginsService extends Service {
	...
	private promptText(): string {
		const lines = [
			...
			"",
			'  await self.plugin.new("rlm-weather", "Report the weather")  // writes it',
			...
			'  await self.plugin.mount("rlm-weather")                      // switch it on',
			'  await self.call("rlmWeather", "hello", "you")               // use it',
			"",
