...
describe("edit summaries", () => {
	...
	test("coalesces direct and Code edits by file", () => {
		const message = assistant([
			{ type: "toolCall", id: "one", name: "edit", arguments: { path: "a.ts" } },
			{ type: "toolCall", id: "two", name: "code", arguments: {} },
		]);
		...
		mergeTurnFileChanges(
			...
			[
				...
				result("two", "code", { diffs: [{ path: "a.ts", oldStr: "new", newStr: "newer" }] }),
			],
			"/tmp",
		);
	...
	test("coalesces home-relative and absolute paths", () => {
		...
		mergeTurnFileChanges(
			...
			[
				result("one", "edit", { diff: "-1 old\n+1 new" }),
				result("two", "code", { diffs: [{ path: absolutePath, oldStr: "new", newStr: "newer" }] }),
			],
			"/tmp",
		);
	...
	test("coalesces canonical paths across a symlinked cwd", () => {
		...
		try {
			...
			const message = assistant([
				{ type: "toolCall", id: "one", name: "code", arguments: {} },
			...
			mergeTurnFileChanges(
				...
				message,
				[
					result("one", "code", { diffs: [{ path: realpathSync(file), oldStr: "old", newStr: "new" }] }),
