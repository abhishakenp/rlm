...
describe("parseArgs", () => {
	...
	describe("tool flags", () => {
		...
		test("parses --tools flag", () => {
			const result = parseArgs(["--tools", "code,dynamic_tool"]);
			expect(result.tools).toEqual(["code", "dynamic_tool"]);
		});

		test("parses -t shorthand", () => {
			const result = parseArgs(["-t", "code,dynamic_tool"]);
			expect(result.tools).toEqual(["code", "dynamic_tool"]);
		});

		test("parses --no-tools with explicit --tools flags", () => {
			const result = parseArgs(["--no-tools", "--tools", "code,dynamic_tool"]);
			expect(result.noTools).toBe(true);
			expect(result.tools).toEqual(["code", "dynamic_tool"]);
		});

		test("parses --no-builtin-tools with explicit --tools flags", () => {
			const result = parseArgs(["--no-builtin-tools", "--tools", "code,dynamic_tool"]);
			expect(result.noBuiltinTools).toBe(true);
			expect(result.tools).toEqual(["code", "dynamic_tool"]);
		});
		...
		test("rejects removed built-in tools", () => {
			...
			expect(result.diagnostics).toContainEqual({
				type: "error",
				message: "Unknown built-in tool(s): read. Available built-in tools: code",
			});
		});
	});
