...
/**
 * Stand-in for the real code tool. Goal calls reach the host over the
 * kernel comm bridge while an code cell executes; this stub mirrors that
 ...
 */
function createFauxCodeTool(sessionRef: { current?: AgentSession }): AgentTool {
	return {
		name: "code",
		label: "code",
...
describe("AgentSession goals", () => {
	...
	async function createGoalHarness(extraTools: AgentTool[] = []): Promise<Harness> {
		...
		const harness = await createHarness({ tools: [createFauxCodeTool(sessionRef), ...extraTools] });
		sessionRef.current = harness.session;
		harnesses.push(harness);
		return harness;
	}

	it("keeps continuing until the model completes the goal through code", async () => {
		...
		harness.setResponses([
			...
			fauxAssistantMessage(fauxToolCall("code", COMPLETE_GOAL_CELL), { stopReason: "toolUse" }),
	...
	it("activates code when a slash goal starts from an inactive tool set", async () => {
		const harness = await createGoalHarness();
		...
		expect(harness.session.getActiveToolNames()).toEqual(["code"]);
		expect(harness.session.goalState).toMatchObject({
			active: false,
			status: "complete",
		});
	...
	it("adds code to the live continuation context when inactive at run start", async () => {
		const harness = await createGoalHarness();
	...
	it("keeps code active on active-goal runtime rebuild", async () => {
		const harness = await createGoalHarness();
		harness.session.handleGoalHostRequest("goal.create", { objective: "finish the active goal" });
	...
	it("lets the model create a persistent goal through code", async () => {
		const harness = await createGoalHarness();
		harness.setResponses([
			fauxAssistantMessage(
				fauxToolCall("code", { code: 'goal.create {"objective": "write a benchmark note"}' }),
				{
					stopReason: "toolUse",
				},
			),
