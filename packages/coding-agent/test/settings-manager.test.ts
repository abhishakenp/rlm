...
describe("SettingsManager", () => {
	...
	describe("error tracking", () => {
		it("should collect and clear load errors via drainErrors", () => {
			...
			const projectSettingsPath = join(projectDir, ".rlm", "settings.json");
		...
		it("should report a new project error when saving after the load error was drained", async () => {
			const settingsPath = join(projectDir, ".rlm", "settings.json");
		...
		it("drains only the requested scope", () => {
			...
			writeFileSync(join(projectDir, ".rlm", "settings.json"), "{ invalid project json");
			const manager = SettingsManager.create(projectDir, agentDir);
	...
	describe("project settings directory creation", () => {
		it("should not create .pi folder when only reading project settings", () => {
			...
			rmSync(join(projectDir, ".rlm"), { recursive: true });

			const manager = SettingsManager.create(projectDir, agentDir);

			expect(existsSync(join(projectDir, ".rlm"))).toBe(false);
		...
		it("should create .pi folder when writing project settings", async () => {
			...
			expect(existsSync(join(projectDir, ".rlm"))).toBe(true);

			expect(existsSync(join(projectDir, ".rlm", "settings.json"))).toBe(true);
		});
	});
