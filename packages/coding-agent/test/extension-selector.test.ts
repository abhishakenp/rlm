...
describe("ExtensionSelectorComponent", () => {
	...
	it("renders a multiline prompt with compact option rows", () => {
		const selector = new ExtensionSelectorComponent(
			[
				"Interrupted Code cell is still running",
				"Ctrl+C sent an interrupt, but the previous cell has not stopped yet.",
				"Waiting preserves the current kernel state. Killing restarts Code and loses in-memory variables.",
			].join("\n"),
		...
		expect(output).toContain("Interrupted Code cell is still running");
		...
		expectTrailingBackground(
			lines.find((line) => line.includes("Interrupted Code cell is still running")),
			"Interrupted Code cell is still running",
		);
