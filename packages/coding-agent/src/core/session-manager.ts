...
// Entry types that can represent user intent (vs. bookkeeping like
...
async function scanSessionInfo(filePath: string, stats: Awaited<ReturnType<typeof stat>>): Promise<SessionInfo | null> {
	try {
		...
		for await (const lineBuffer of readLinesAsBuffers(filePath)) {
			...
			// Keep the latest recap/verdict so sessions don't all show as
...
export class SessionManager {
	 ...
	 * lets the async path skip the synchronous re-read.
	 */
	 ...
	 * only bookkeeping (session_state, agent_status, git_state) or
	 * the default model/thinking entries every new session is created with. Used by
	 * the discard guard to decide whether a message-less draft is safe to
