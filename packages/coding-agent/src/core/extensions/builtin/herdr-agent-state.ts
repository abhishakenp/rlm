 ...
 * loader invokes per session load — inside the client-env window —
 * so each session captures its own pane identity.
 *
...
function herdrAgentStateExtensionImpl(pi: ExtensionAPI, getLoadedExtensionPaths: () => string[]): void {
	...
	// session load, inside the client-env window, so these reflect the
	// session's own Herdr pane rather than the process startup environment.
	...
	pi.on("session_start", (_event, ctx) => {
		...
		// A reload can re-create this reporter mid-turn (runtime-driven reloads
