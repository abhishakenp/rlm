...
const CODE_CONTROL_PROMPT = [
	"The `code` tool is the agent's long-lived notebook: a persistent JavaScript execution environment for reasoning, context management, state, tool orchestration, and recursive subcalls. Use it to keep intermediate variables, inspect and transform outputs, and write small helper functions. The code tool ONLY runs JavaScript — no Python, no Ruby, no other language.",
	"",
	"When the user asks for code in another language (e.g. 'write binary search in Python'), write the JavaScript equivalent in the code tool and run it. If the user specifically needs Python output, write the code to a file with fs.writeFileSync and run it with !python — but default to JavaScript for everything.",
	"",
