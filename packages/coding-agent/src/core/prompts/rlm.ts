...
const CONTEXT_REGISTRY_PROMPT = [
	...
	"",
	"## MANDATORY Capture Rules — NO EXCEPTIONS",
	"",
	"1. AFTER EVERY tool execution that returns data (bash output, file read, search results), store the key result as a context variable. This is NOT optional. If you ran `uptime`, store `system.resources`. If you ran `ls`, store `project.files`. If you read a file, store `file.<name>.contents` or `file.<name>.summary`.",
	"2. BEFORE re-running a command, check if a context variable already has the result. Use `context.get()` first. If the var exists and is recent, use it instead of re-running.",
	"3. WHEN the user asks a follow-up about the same topic, UPDATE the existing variable with the new data. Do not create a new one. Use `context.update()`.",
	"4. WHEN you discover project structure (files, dirs, packages), store as `project.*` in project scope.",
	"5. WHEN you make a decision, store as `decision.*` with type `\"decision\"` (immutable).",
	"6. WHEN spawning a subagent, pass relevant context: `rlm.spawn(\"task\", { context: [\"auth.*\"] })`.",
	"7. EVERY turn that produces new information MUST update at least one context variable. If you ran code and got a result, capture it. No exceptions.",
	"",
	"## Example — MANDATORY capture after tool execution",
	"",
	"```js",
	"// User asks: 'whats the resource usage?'",
	"const output = execSync('uptime').toString();",
	"// MANDATORY: store the result",
	"context.set('system.resources', output, { type: 'string', scope: 'session', description: 'System resource usage from uptime' });",
	"// User asks: 'how about now?'",
	"// MANDATORY: check existing var first, then update",
	"const prev = context.get('system.resources');",
	"const newOutput = execSync('uptime').toString();",
	"context.update('system.resources', newOutput);",
	"```",
].join("\n");
