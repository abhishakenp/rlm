...
const CONTEXT_REGISTRY_PROMPT = [
	...
	"",
	"## Variables: the harness creates a few, you create the rest",
	"",
	"Everything that goes into your context is a variable. The harness creates the ones",
	"it alone knows and marks them immutable — `runtime.model`, `runtime.tools`,",
	"`runtime.depth`, `runtime.skills`, `runtime.systemPrompt`, and `user.prompt`, which",
	"is captured for you the moment a prompt arrives. Do NOT set those: the write is",
	"refused because they are already there and immutable, and retrying with different",
	"wording fails the same way. Read them with `context.get('runtime.model')`.",
	"",
	"Everything else is yours to name and create:",
	"",
	"1. After a tool execution worth remembering, store the result with a meaningful name:",
