...
- skill: installed skill. Skill create/update edits MUST include a \`reference\` object with \`{"type":"js"}\`, a JS import, and a callable or call pattern; they also MUST include an \`arguments\` object describing accepted inputs, required fields, defaults, and constraints. Use \`{}\` for \`arguments\` only when the callable truly needs no external inputs. Include the RLM-native call form \`await <skill_import>(...)\`.
  ...
  "edits": [
    {
      ...
      "reference": {"type": "js", "import": "package.module", "callable": "function_name", "call_pattern": "await function_name(...)"},
...
export function formatHarnessStateForPrompt(
	...
	const lines = [
		...
		"",
		includeCodeExamples
			? "Call contract: read each installed skill's SKILL.md and call its documented module function in Code; do not assume a `.run` entrypoint. Use `<skill_import> ...` in shell when a CLI exists. Continual harness skill entries are JS skills with an explicit JS `reference` and `arguments` contract. Spawn a continual harness subagent spec by composing a concise task prompt and calling `handle = await rlm('sub-task')`; admission returns immediately with `rlm_child_id`, `name`, `session_dir`, and `model`, never the child's answer. Results arrive only through explicit `agent_message` replies or files; children reply with `await agent_message.send(message, receiver_role='parent')`. Use `await rlm.list_subagents()` to recover direct child handles and `await agent_message.send(..., receiver_role='child', receiver_name=handle.name)` for follow-ups. Do not invent wrappers such as `call_skill(...)`, `run_subagent(...)`, or named subagent registries."
			: options.includeShellExamples
				? "Call contract: use installed skills as shell commands when available (for example `<skill_import> ...`). Continual harness entries are routing/context hints only in sessions without Code; do not use `await`, `asyncio`, or `rlm` examples unless the prompt also documents an Code kernel."
				: "Call contract: continual harness entries are routing/context hints only in sessions without Code or shell access; do not use `await`, `asyncio`, `rlm`, or shell skill commands unless the prompt also documents those interfaces.",
		"",
	];
...
function validateEdit(edit: RefinementEdit, computedId?: string): string | undefined {
	...
	if (edit.action !== "delete" && edit.kind === "skill") {
		...
		const hasImport =
			...
			(typeof reference.js_import === "string" && reference.js_import.length > 0);
		const hasCallable =
		...
		if (!hasImport) {
			return `${edit.action} skill requires js import`;
		}
		if (!hasCallable) {
...
export async function planRefinement(
	...
	// final text, which makes otherwise successful /refine calls fail parsing.
