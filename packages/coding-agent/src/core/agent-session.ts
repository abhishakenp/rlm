...
export class AgentSession {
	...
	// the runtime does the same by leaving the child session resident in its registry.
	...
	/** Latest recap for this session, written by the summarizer; read by a parent to label its child snapshots. */
	 ...
	 * when the session is created outside the interactive mode.
	 */
	 ...
	 * pi.exec() subprocesses. The function is read at exec time, so a host
	 * can update the underlying value per attach without rebinding.
	 */
	...
	async reload(): Promise<void> {
		...
		// Re-read auth.json: a login saved by another process must be
	...
	private _buildRlmSubagentList(listedAgents?: AgentSessionMessageListResult): RlmListSubagentsResult {
		const listedChildren = new Map<string, AgentSessionMessageAgentSummary>();
		...
		if (parentActiveSessionId) {
			for (const agent of listedAgents.agents) {
				if (
					...
					listedChildren.set(agent.rlmChildId, agent);
				}
			}
		}
		...
		for (const run of this._activeRlmChildRuns.values()) {
			...
			const listedChild = listedChildren.get(run.id);
			subagents.push({
				rlm_child_id: run.id,
				active_session_id: listedChild?.activeSessionId ?? null,
				session_id: listedChild?.sessionId ?? run.session?.sessionId ?? null,
				session_name: listedChild?.sessionName ?? run.session?.sessionName ?? run.sessionName,
		...
		for (const [childId, childSession] of this._rlmChildSessions) {
			...
			const listedChild = listedChildren.get(childId);
			...
			subagents.push({
				...
				active_session_id: listedChild?.activeSessionId ?? null,
				session_id: listedChild?.sessionId ?? childSession.sessionId,
				session_name:
					listedChild?.sessionName ?? childSession.sessionName ?? createDefaultRlmSubagentSessionName("", childId),
				session_dir: sessionDir,
				status: "completed",
			});
			recorded.add(childId);
		}
		for (const [childId, listedChild] of listedChildren) {
			if (
				recorded.has(childId) ||
				this._deletingRlmChildren.has(childId) ||
				this._deletedRlmChildIds.has(childId) ||
				this._rlmChildCleanupFailures.has(childId) ||
				!listedChild.sessionDir
			) {
				continue;
			}
			subagents.push({
				rlm_child_id: childId,
				active_session_id: listedChild.activeSessionId,
				session_id: listedChild.sessionId,
				session_name: listedChild.sessionName ?? createDefaultRlmSubagentSessionName("", childId),
				session_dir: listedChild.sessionDir,
				status: listedChild.rlmChildRegistryStatus === "completed" ? "completed" : "error",
			});
		}
		return { subagents };
	}
	...
	async deleteRlmSubagent(target: string): Promise<RlmDeleteSubagentResult> {
		...
		// them hidden immediately while the async listing checks for a
		...
		if (localMatches[0]) {
			...
			return this._trackRlmSubagentDeletion(subagent, async () => {
				...
				const parentActiveSessionId = listedAgents?.current?.activeSessionId;
				const listedChild = listedAgents?.agents.find(
					(agent) =>
						agent.rlmChildId === subagent.rlm_child_id && agent.parentActiveSessionId === parentActiveSessionId,
				);
				const resolvedSubagent = listedChild
					? {
							...subagent,
							active_session_id: listedChild.activeSessionId,
							session_id: listedChild.sessionId,
							session_name: listedChild.sessionName ?? subagent.session_name,
						}
					: subagent;
	 ...
	 * agent messaging can keep addressing it. Returns false (and disposes
	...
	private async _startFleetRlmChild(prompt: string, kwargs: Record<string, unknown>): Promise<RlmSpawnHandle> {
		const { buildRuntimeRegistry, LocalRuntime } = await import("./fleet-runtime/index.js");
