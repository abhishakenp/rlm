...
function mergeSubagentSnapshot(
	...
	return {
		...
		// Active updates may omit a previously known session id, but a
...
export interface InteractiveModeOptions {
	 ...
	 * which also covers direct attaches where the agents view was never shown.
	 */
	agentsViewOwnsStartupNotices?: boolean;
	/** Persisted RLM depth supplied by the SessionSummary. */
	sessionDepth?: number;
	/** Whether the unified session/catalog projection had any direct children. */
...
export class InteractiveMode {
	...
	async run(): Promise<InteractiveModeRunResult> {
		...
		const deliverStartupPrompts = async () => {
			...
			for (let next = 0; next < startupPrompts.length; ) {
				...
				try {
					...
					// An uncertain admission may already be session-owned. Retrying
		...
		const showDeferredStartupNotifications = () => {
			...
			// The agents view owns these for background sessions. When there is no agents view,
	...
	private getModelFallbackWarningAction(modelFallbackMessage: string | undefined): ModelFallbackWarningAction {
		...
		// visible to the session process, or added after the snapshot was taken).
	...
	private async renderCurrentSessionState(): Promise<void> {
		// Replacement events own the session-scoped command catalog. The connection
		...
		this.unsubscribe = this.agentConnection.subscribe(async (event) => {
			try {
				if (event.type === "session_event") {
					...
					this.showStatus(
						event.status === "connected" ? "Connection restored" : "Connection lost; reconnecting…",
	...
	private async openScopedAgentsView(): Promise<void> {
		if (!this.options.returnToAgentsView) {
			this.focusEditor();
			this.showStatus("The agents view is not available in this mode");
			return;
		}
	...
	private moveQueueSelection(direction: -1 | 1): void {
		...
		void this.enqueueQueueMutation(async () => {
			...
			} else if (status === "unsupported") this.showStatus("Queue editing is not supported by this connection");
	...
	private async handleUpdateCommand(args: string): Promise<void> {
		...
		const updateResult = spawnSync(
			process.execPath,
			[...process.execArgv, entrypoint, "update", ...updateArgs],
			{
				stdio: "inherit",
				cwd: updateCwd,
				env: updateEnv,
	...
	private handleLogsCommand(): void {
		...
		info += `\n${theme.fg("dim", "Agent-open failures log to client-errors.log.")}`;
