 ...
 * receive AgentSessionRuntime, AgentSession, SessionManager, in-process
 * event emitters, or executable callbacks through
...
/** `unsupported` means an older remote connection lacks queued-message mutation. */
...
export interface AgentConnectionRlmChildAgentSnapshot {
	id: string;
	parentId?: string;
	/** Child active-session id, for direct attachment. */
	activeSessionId?: string;
	/** Stable child name for addressing and display. */
	sessionName?: string;
