...
function rejectRemovedCommand(args: string[]): PublicCommandResult {
	...
	if (command === "app" && subcommand === "update") {
...
async function runInternalAgentCommand(command: string, _args: string[]): Promise<PublicCommandResult> {
	return fail(`The "${command}" command is not available in this build.`);
}

async function runNestedAgentCommand(
	parent: string,
	_internalCommand: string,
	args: string[],
	...
	return fail(`The "${parent} ${subcommand}" command is not available in this build.`);
}

async function runStatus(_args: string[]): Promise<PublicCommandResult> {
	return fail('The "status" command is not available in this build.');
}

async function runDoctor(_args: string[]): Promise<PublicCommandResult> {
	return fail('The "doctor" command is not available in this build.');
}

async function runShutdown(_args: string[]): Promise<PublicCommandResult> {
	return fail('The "shutdown" command is not available in this build.');
}
